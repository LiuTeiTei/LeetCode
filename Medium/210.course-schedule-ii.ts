...
function findOrder(numCourses: number, prerequisites: number[][]): number[] {
    ...
    for (let i = 0; i < numCourses; i++) {
        if (inDegreeArr[i] === 0) queque.push(i)
    }

    const result: number[] = []
    while(queque.length) {
        const curr = queque.shift()!
        result.push(curr)

        const currInDegreeArr = inDegreeMap.get(curr)
        for (let i = 0; i < currInDegreeArr?.length; i++) {
            inDegreeArr[currInDegreeArr[i]] -= 1
            if (inDegreeArr[currInDegreeArr[i]] === 0) queque.push(currInDegreeArr[i])
        }
    }
